...
const menuItems = computed(() => {
  ...
  if (replyMode.value === REPLY_EDITOR_MODES.REPLY && draftMessage.value) {
    items.push({
      label: t('JIVO.COPILOT.MENU.IMPROVE_REPLY'),
      key: 'improve',
  ...
  if (draftMessage.value) {
    items.push(
      {
        label: t('JIVO.COPILOT.MENU.CHANGE_TONE.TITLE'),
        key: 'change_tone',
        icon: 'i-fluent-sound-wave-circle-sparkle-24-regular',
        subMenuItems: [
          {
            label: t('JIVO.COPILOT.MENU.CHANGE_TONE.OPTIONS.PROFESSIONAL'),
            key: 'professional',
          },
          {
            label: t('JIVO.COPILOT.MENU.CHANGE_TONE.OPTIONS.CASUAL'),
            key: 'casual',
          },
          {
            label: t('JIVO.COPILOT.MENU.CHANGE_TONE.OPTIONS.STRAIGHTFORWARD'),
            key: 'straightforward',
          },
          {
            label: t('JIVO.COPILOT.MENU.CHANGE_TONE.OPTIONS.CONFIDENT'),
            key: 'confident',
          },
          {
            label: t('JIVO.COPILOT.MENU.CHANGE_TONE.OPTIONS.FRIENDLY'),
            key: 'friendly',
          },
        ],
      },
      {
        label: t('JIVO.COPILOT.MENU.GRAMMAR'),
...
const generalMenuItems = computed(() => {
  ...
  if (replyMode.value === REPLY_EDITOR_MODES.REPLY) {
    items.push({
      label: t('JIVO.COPILOT.MENU.SUGGESTION'),
  ...
  items.push({
    label: t('JIVO.COPILOT.MENU.SUMMARIZE'),
    key: 'summarize',
    icon: 'i-fluent-text-bullet-list-square-sparkle-32-regular',
  });

  items.push({
    label: t('JIVO.COPILOT.MENU.ASK_COPILOT'),
    key: 'ask_copilot',
  ...
  if (isAdmin.value) {
    items.push({
      label: t('JIVO.COPILOT.MENU.LEARN_FROM_CONVERSATION'),
      key: 'learn_from_conversation',
      icon: 'i-lucide-brain',
    });
  }
