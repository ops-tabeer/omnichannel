...
#  auto_reassigned        :boolean          default(FALSE), not null
...
#  index_conversations_on_account_id_and_last_activity_at  (account_id,last_activity_at DESC)
#  index_conversations_on_account_inbox_last_activity_at   (account_id,inbox_id,last_activity_at DESC)
...
class Conversation < ApplicationRecord
  ...
  def determine_conversation_status
    ...
    self.status = :pending if inbox.active_bot? || inbox.active_jivo_assistant?
  end

  def handle_campaign_status
    # If campaign has no sender (bot-initiated) and inbox has active bot, let bot handle it
    self.status = :pending if campaign.sender_id.nil? && (inbox.active_bot? || inbox.active_jivo_assistant?)
  end
