...
class JivoCustomTool < ApplicationRecord
  include JivoToolable
  include JivoSafeEndpointValidatable
  ...
  def find_unique_slug(base_slug)
    ...
    5.times do
      candidate = "#{base_slug}#{NAME_SEPARATOR}#{SecureRandom.alphanumeric(6).downcase}"
      return candidate unless slug_exists?(candidate)
    end

    raise ActiveRecord::RecordNotUnique, 'Unable to generate unique slug after 5 attempts'
  end
