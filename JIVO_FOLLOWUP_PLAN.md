...
- **Post-review hardening (from /code-review high):**
...
- ✅ **Token + cost tracking:** `input_tokens`/`output_tokens` columns (migration `20260701010000`) summed from the real OpenAI `usage` block via reusable `BaseTaskService#token_usage`. Cost is computed **per call at that call's own model** (`Jivo::ModelPricing.cost`) and accumulated into a `cost_micros` column (migration `20260701020000`), so the monthly total is correct even when an account's assistants use different models; the Super Admin view just reads `estimated_cost` (no model guessing). Both helpers are deliberately generic so the **main JIVO bot can reuse them later**.
- ✅ **Display:** `app/views/super_admin/accounts/_jivo_ai_usage.html.erb` rendered at the bottom of the Super Admin account show page (mirrors the `_seed_data`/`_reset_cache` partials) — shows current-month total, per-action breakdown, tokens in/out, and an estimated cost (labelled "idle follow-up only, approximate").
