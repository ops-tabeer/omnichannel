...
- ✅ **Token + cost tracking:** `input_tokens`/`output_tokens` columns (migration `20260701010000`) summed from the real OpenAI `usage` block via reusable `BaseTaskService#token_usage`. `Jivo::ModelPricing` (per-model $/1M map + `cost`) gives a ballpark USD estimate at the assistant's model rate. Both helpers are deliberately generic so the **main JIVO bot can reuse them later**.
